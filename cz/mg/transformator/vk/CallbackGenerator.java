...
public class CallbackGenerator extends javax.swing.JFrame {
    ...
    */
    ...
    private void generateComplex(){
        ...
        output = output.replace("PARAMETERS", genParameters(input));
        output = output.replace("RVAL", genRval(input));
    ...
    private String genName(String input){
        String[] lines = input.split("\\n");
        return lines[0].split(" ")[3].replace("*", "").replace(")(", "");
    }
    
    private String genParameters(String input){
        String[] lines = input.split("\\n");
        String properties = "";
        for(int i = 1; i < lines.length; i++){
            String nline = lines[i].trim().replace(");", "");
            while(nline.contains("  ")) nline = nline.replace("  ", " ");
            String[] parts = nline.replace("const ", "").split(" ");
            String type = parts[0];
            String name = parts[1];
            properties = properties + "            " + type + " " + name + "\n";
        }
        return properties.substring(0, properties.length()-1);
    }

    private String genRval(String input){
        String[] lines = input.split("\\n");
        return lines[0].split(" ")[1];
    }
    
    private String readAll(){
        InputStream stream = getClass().getResourceAsStream("CallbackSample.txt");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(CallbackGenerator.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new CallbackGenerator().setVisible(true);
            }
        });
    }
